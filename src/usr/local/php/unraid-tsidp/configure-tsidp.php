...
if ( ! $key) {
    fwrite(STDERR, "Failed to create API key with unraid-api\n");
    safeExit(1, $keyName);
}
...
foreach ($settings['sso']['providers'] as &$prov) {
    if (is_array($prov) && isset($prov['id']) && $prov['id'] === 'tsidp') {
        ...
        if ($prov['issuer'] === $issuer) {
            echo "tsidp provider already exists and configured correctly. No changes made.\n";
            safeExit(0, $keyName);
        }

        echo "Updating tsidp URL\n";
        $prov['issuer'] = $issuer;
        break;
    }
}
...
echo "tsidp provider configured successfully.\n";
