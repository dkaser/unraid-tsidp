#!/usr/bin/env php
<?php

// configure-tsidp.php
// Configure tsidp as an OIDC provider in Unraid via GraphQL API

// 1. Find CliInternal key
$keyDir   = '/boot/config/plugins/dynamix.my.servers/keys';
$keyFile  = null;
$keyValue = null;
foreach (glob("{$keyDir}/*.json") ?: [] as $file) {
    $data = (array)json_decode(file_get_contents($file) ?: '{}', true);
    if (isset($data['name']) && $data['name'] === 'CliInternal' && isset($data['key'])) {
        $keyFile  = $file;
        $keyValue = $data['key'];
        break;
    }
}
if ( ! $keyValue) {
    fwrite(STDERR, "CliInternal key not found\n");
    exit(1);
}

// 2. Read TSIDP_PORT from tsidp.cfg
$cfgFile   = '/boot/config/plugins/tsidp/tsidp.cfg';
$tsidpPort = null;
if (file_exists($cfgFile)) {
    $ini = parse_ini_file($cfgFile);
    if (isset($ini['TSIDP_PORT'])) {
        $tsidpPort = $ini['TSIDP_PORT'];
    }
}
if ( ! $tsidpPort) {
    fwrite(STDERR, "TSIDP_PORT not found in tsidp.cfg\n");
    exit(1);
}

// 3. Get Tailscale FQDN
$tailscaleJson = shell_exec('tailscale status -json 2>/dev/null');
if ( ! is_string($tailscaleJson) || $tailscaleJson === '') {
    fwrite(STDERR, "Failed to get tailscale status\n");
    exit(1);
}
$tailscale = json_decode($tailscaleJson, true);
if ( ! is_array($tailscale) || ! isset($tailscale['Self']) || ! is_array($tailscale['Self']) || ! isset($tailscale['Self']['DNSName']) || ! is_string($tailscale['Self']['DNSName'])) {
    fwrite(STDERR, "Self.DNSName not found in tailscale status\n");
    exit(1);
}
$fqdn = rtrim((string)$tailscale['Self']['DNSName'], '.');

// 4. GraphQL helper
function graphql(string $query, array $variables = [], string $key = ""): array|null
{
    $sock    = '/var/run/unraid-api.sock';
    $headers = [
        'Host: localhost',
        'Content-Type: application/json',
    ];
    if ($key) {
        $headers[] = "X-Api-Key: {$key}";
    }
    $body = [ 'query' => $query ];
    if ($variables) {
        $body['variables'] = $variables;
    }
    $jsonBody = json_encode($body);

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_UNIX_SOCKET_PATH, $sock);
    curl_setopt($ch, CURLOPT_URL, 'http://localhost/graphql');
    curl_setopt($ch, CURLOPT_POST, true);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $jsonBody ?: "");
    curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_FAILONERROR, false);
    $result = curl_exec($ch);
    if ($result === false) {
        curl_close($ch);
        return null;
    }
    curl_close($ch);
    return is_string($result) ? (array)json_decode($result, true) : null;
}

// 5. Query unified settings
$unifiedQuery = 'query Unified { settings { unified { values } } }';
$resp         = graphql($unifiedQuery, [], $keyValue);
if ( ! is_array($resp) || ! isset($resp['data']['settings']['unified']['values'])) {
    fwrite(STDERR, "Failed to get unified settings\n");
    exit(1);
}
$settings = $resp['data']['settings']['unified']['values'];
if ( ! is_array($settings) || ! isset($settings['sso']) || ! is_array($settings['sso'])) {
    fwrite(STDERR, "Invalid unified settings format\n");
    exit(1);
}

// 6. Check for tsidp provider
if ( ! isset($settings['sso']['providers']) || ! is_array($settings['sso']['providers'])) {
    $settings['sso']['providers'] = [];
}
$exists = false;
foreach ($settings['sso']['providers'] as $prov) {
    if (is_array($prov) && isset($prov['id']) && $prov['id'] === 'tsidp') {
        $exists = true;
        break;
    }
}
if ($exists) {
    echo "tsidp provider already exists. No changes made.\n";
    exit(0);
}

// 7. Add tsidp provider
$settings['sso']['providers'][] = [
    'id'                    => 'tsidp',
    'name'                  => 'Tailscale',
    'clientId'              => 'tailscale',
    'clientSecret'          => '',
    'issuer'                => "https://{$fqdn}:{$tsidpPort}",
    'scopes'                => ['openid', 'profile', 'email'],
    'authorizationRuleMode' => 'or',
    'authorizationRules'    => [],
    'buttonText'            => '',
    'buttonVariant'         => 'outline',
    'buttonStyle'           => 'background-color: #ffffff;',
    'buttonIcon'            => 'data:image/png;base64,',
    'isProtected'           => false,
    'authorizationMode'     => 'simple',
    'simpleAuthorization'   => [
        'allowedDomains' => [],
        'allowedEmails'  => [],
        'allowedUserIds' => []
    ]
];

// 8. Update settings
$mutation   = 'mutation Mutation($input: JSON!) { updateSettings(input: $input) { restartRequired } }';
$vars       = [ 'input' => $settings ];
$updateResp = graphql($mutation, $vars, $keyValue);
if ( ! $updateResp || ! isset($updateResp['data']['updateSettings'])) {
    fwrite(STDERR, "Failed to update settings\n");
    exit(1);
}
echo "tsidp provider added successfully.\n";
